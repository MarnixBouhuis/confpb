...
func TestInt64(t *testing.T) {
	...
	tests := []*struct {
		...
		},
		{
			...
			expectedResult:    0,
			expectedHasResult: true,
			expectedErr: fmt.Errorf("failed to process environment variable \"FOO\": invalid value \"\", unable to parse int64: %w", &strconv.NumError{
				Func: "ParseInt",
				Num:  "",
		...
		},
		{
			...
			expectedResult:    0,
			expectedHasResult: true,
			expectedErr: fmt.Errorf("failed to process environment variable \"FOO\": invalid value \"not-a-number\", unable to parse int64: %w", &strconv.NumError{
				Func: "ParseInt",
				Num:  "not-a-number",
				Err:  errors.New("invalid syntax"),
			}),
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			result, hasResult, err := scan.Int64(test.args.envKey)
