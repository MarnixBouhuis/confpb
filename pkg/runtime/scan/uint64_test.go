...
func TestUint64(t *testing.T) {
	...
	tests := []*struct {
		...
		},
		{
			...
			expectedResult:    0,
			expectedHasResult: true,
			expectedErr: fmt.Errorf("failed to process environment variable \"FOO\": invalid value \"\", unable to parse uint64: %w", &strconv.NumError{
				Func: "ParseUint",
				Num:  "",
		...
		},
		{
			...
			expectedResult:    0,
			expectedHasResult: true,
			expectedErr: fmt.Errorf("failed to process environment variable \"FOO\": invalid value \"not-a-number\", unable to parse uint64: %w", &strconv.NumError{
				Func: "ParseUint",
				Num:  "not-a-number",
				Err:  errors.New("invalid syntax"),
			}),
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			result, hasResult, err := scan.Uint64(test.args.envKey)
