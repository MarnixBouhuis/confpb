...
func TestFloat(t *testing.T) {
	...
	tests := []*struct {
		...
		},
		{
			...
			expectedResult:    0,
			expectedHasResult: true,
			expectedErr: fmt.Errorf("failed to process environment variable \"FOO\": invalid value \"\", unable to parse float: %w", &strconv.NumError{
				Func: "ParseFloat",
				Num:  "",
		...
		},
		{
			...
			expectedResult:    0,
			expectedHasResult: true,
			expectedErr: fmt.Errorf("failed to process environment variable \"FOO\": invalid value \"not-a-number\", unable to parse float: %w", &strconv.NumError{
				Func: "ParseFloat",
				Num:  "not-a-number",
				Err:  errors.New("invalid syntax"),
			}),
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			result, hasResult, err := scan.Float(test.args.envKey)
